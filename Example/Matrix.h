...
template< typename TYPE = int >
  class Matrix
  {
    ...
    public:
      ...
      //-------------------------------------------------------------
      // Add a row the the end of the matrix
      //-------------------------------------------------------------
      void add()
      { vector<TYPE> temp;
	...
	rows++;	// Increment #rows
      ...
      //-------------------------------------------------------------
      // Add an element to the last row 
      //-------------------------------------------------------------
      void add(TYPE elt)
      { matrix[rows-1].push_back(elt);
        if (rows==1) 
	  columns++;
      }
...
//-----------------------------------------------------------------------------
// Stream operator used to convert file of elements into array
//	Matrix is assumed to be in file in n x m form
//-----------------------------------------------------------------------------
template< typename TYPE >
  ifstream & operator>>
  (
    ...
    // A non-const matrix.
    Matrix< TYPE > & TheMatrix
  )
  {	  // Need vector for each row
	  vector<TYPE> temp;
	  unsigned rowNum=0,colNum=0;
	  while (!stream.eof()) {
	    // Add vector for the row
	    TheMatrix.add();  // Add a row vector to the matrix
	    // Read line of file.
	    string line;
	    getline(stream,line);
	    // Use string stream to tokenize and convert
	    istringstream rowElts(line);
	    TYPE elt;
	    // Fill the row
	    while (rowElts >> elt) {
	      // Add the new element to the matrix
	      TheMatrix.add(elt);
	    }  
	  }  // While
   } 
